...
##########
conds <- stDat$conds
facetY <- calcInfo$fractions$y_facets
for(ct in names(calcInfo)){
    log_debug(paste("Calculation:",ct))
#pdfHeight = 8.5
pdfWidth = 12
outPDF = file.path(cfg$statistics_detail_dir, "3d_3d_all_filtered_conditions.pdf")
    suppressMessages(
pg <-   plotSeparateCohortVsIndividual(finalDat[[paste0("filtered_",ct)]], #%>% filter(`Cell State ID` %in% idOrder), 
                                       allIndivDat[[ct]], # %>% filter(`Cell State ID` %in% idOrder),
            ...
            ggsave(pg, height = pdfHeight, width = pdfWidth, units = "in",

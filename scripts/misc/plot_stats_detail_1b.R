...
#### TEMP
ids <- c(509, 190, 66, 33, 438, 500, 506, 486, 425, 328, 361, 393, 187, 54, 20, 87, 276, 42)
stDat$conds <- stDat$conds %>%
               filter(as.numeric(`Cell State ID`) %in% ids)
stDat$conds$OuterFacet = ""
stDat$conds$Phenotype = NA
stDat$conds$Phenotype[as.numeric(stDat$conds$`Cell State ID`) %in% c(509, 190, 66, 33, 438)] <- "Cell Identity"
stDat$conds$Phenotype[as.numeric(stDat$conds$`Cell State ID`) %in% c(500, 506)] <- "B2M,MHCI,MHCII"
stDat$conds$Phenotype[as.numeric(stDat$conds$`Cell State ID`) %in% c(486, 425, 328, 361, 393)] <- "B7H3"
stDat$conds$Phenotype[as.numeric(stDat$conds$`Cell State ID`) %in% c(187, 54, 20, 87, 276)] <- "CD27"
stDat$conds$Phenotype[as.numeric(stDat$conds$`Cell State ID`) %in% c(42)] <- "PD1,LAG3,TIM3,KI67"

cfg$calculations$fractions$y_facets <- c("OuterFacet", "Phenotype")
cfg$facet_order <- list(OuterFacet = "", 
                        Phenotype = c("Cell Identity", 
                                      "B2M,MHCI,MHCII", 
                                      "B7H3",
                                      "CD27",
                                      "PD1,LAG3,TIM3,KI67")) 
cfg$pdfWidth <- 12
rel_widths$fractions <- c(1.5, 0.6, 0.75)
############
